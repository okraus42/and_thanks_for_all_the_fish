...
/*   Updated: 2024/03/05 15:30:18 by okraus           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_remove_collectible(t_max *max, int x, int y)
{
	...
	i = 0;
	max->score += 5000;
	ft_soundeffect(max->ev, COLLECT, 128);
	while (i < max->map->ct)
	{
		if (max->map->cx[i] == x && max->map->cy[i] == y)
		{
			max->img->ci[i].enabled = 0;
			max->img->c2i[i].enabled = 0;
		}
		i++;
	}
}
...
void	ft_open_door(t_max *max)
{
	...
	max->score += 50000;
	ft_soundeffect(max->ev, EXITUNLOCK, 256);
}
...
void	ft_check_door(t_max *max)
{
		...
		&& max->map->py == max->map->xy)
	{
		...
		max->time += 60000;
		max->lives += 1;
		ft_soundeffect(max->ev, LEVELWIN, 256);
